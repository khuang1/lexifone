...
*  througt contract.
*
*  File Name: getLexifonePassAdmin.py
*
*  History Log:
*    Created: 10/20/2015
*    Author:  Kevin Huang
...
def lambda_handler(event, context):
    ...
    operations = {
        ...
        'delPass':  lambda x: dynamo.delete_item (
                                Key={'ID': clientVal['user']},
                                Expected={  'ID': {'Value': clientVal['user']}}
                            ),
        ...
        'getAvail': lambda x: dynamo.query(
                                IndexName='Status-index',
                                Select='SPECIFIC_ATTRIBUTES',
                                AttributesToGet=['ID', 'Password'],
                                KeyConditions={
        ...
        'getUsed':  lambda x: dynamo.query(
                                IndexName='Status-index',
                                Select='ALL_ATTRIBUTES',
                                KeyConditions={
                                    'Status': {
                                        'AttributeValueList': ['in-use'],
                                        'ComparisonOperator': 'EQ'
                                    }
                                }
                            ),
